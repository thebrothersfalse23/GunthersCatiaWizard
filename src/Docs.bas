...
Public Sub gunthersCatiaWizardDocs()
    ...
    '   getSelectedProducts([firstSelection As Boolean = False]) As Variant
    '   generateTDSingle(selectedProduct As Product, prefix As String, startOnSelected As Boolean, protectRefDocs As Boolean)
    ...
    ' getSelectedProducts([firstSelection As Boolean = False]) As Variant
    '   Description:
    '     Returns either the first selected Product (if firstSelection = True), or all selected Products as a Collection.
    '     Returns Nothing if no Product is selected and firstSelection = True.
    '   Acceptable args:
    '     firstSelection [Boolean, Optional] - True for a single Product, False for all (default: False)
    '   Usage:
    '     Dim prod As Product
    '     Set prod = getSelectedProducts(True)
    ...
    ' generateTDSingle(selectedProduct As Product, prefix As String, startOnSelected As Boolean, protectRefDocs As Boolean)
    '   Description:
    '     Executes the single tool design logic on the selected Product.
    '     Applies the given serial prefix and options to the selected Product.
    '   Acceptable args:
    '     selectedProduct [Product] - The Product object to operate on (from getSelectedProducts(True)).
    '     prefix [String] - Serial number prefix to use for renaming/numbering.
    '     startOnSelected [Boolean] - If True, numbering starts on the selected product; otherwise, starts elsewhere.
    '     protectRefDocs [Boolean] - If True, reference documents are protected (PREFIX_REF_DOCUMENT logic).
    '   Usage:
    '     Dim prod As Product
    '     Set prod = getSelectedProducts(True)
    '     generateTDSingle prod, "VG201144", True, False
    '
    ' generateTDSequential()
    '   Description:
    '     Executes the sequential tool design logic for the current selection/context.
    '     Typically operates on the current ProductDocument and user options.
    '   Acceptable args:
    '     (none)
    '   Usage:
    '     generateTDSequential
